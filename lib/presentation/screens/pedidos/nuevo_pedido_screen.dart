...
class NuevoPedidoScreen extends StatefulWidget {
  /// Si se pasa, la pantalla carga el pedido existente para editarlo.
  final int? editPedidoId;

  const NuevoPedidoScreen({super.key, this.editPedidoId});
...
class _NuevoPedidoScreenState extends State<NuevoPedidoScreen> {
  ...
  Future<void> _loadForEdit() async {
    ...
    final cabecera = await repo.getById(widget.editPedidoId!);
    ...
    // Reconstruir ItemPedido usando el precio guardado (no recalcular)
  ...
  Future<bool> _onWillPop() async {
    ...
    // Hay items pero no se pudo auto-guardar (cliente no seleccionado)
    final discard = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        title: const Text('Pedido sin guardar'),
        content: const Text(
  ...
  Future<void> _selectCliente() async {
    ...
    if (cliente != null && mounted) {
      context.read<PedidoProvider>().setCliente(cliente);
      _tryAutoSave();
    }
  }
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(_isEditing
              ? 'Editar Pedido #${widget.editPedidoId}'
              : 'Nuevo Pedido'),
          actions: [
          ...
          children: [
            ...
            if (provider.saving)
              ...
              ElevatedButton.icon(
                ...
                label: Text(_isEditing ? 'ACTUALIZAR PEDIDO' : 'GUARDAR PEDIDO'),
              ),
