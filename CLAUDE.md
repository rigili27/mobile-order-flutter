...
| `deposito_asignado` | `<id>` / *(ausente)* | Modo API. Si el vendedor tiene un depósito asignado en el ERP: `Parametros.depositoAsignado`. La app fija el filtro de depósito a ese y lo oculta (Artículos, Control de stock), y cada renglón de pedido sale de ese depósito. Ausente = el vendedor elige y ve el stock del central. |
| `permite_stock_negativo` | `true` (default) / `false` | Modo API. `false` (solo con `deposito_asignado`) → la app no deja agregar un renglón sin stock suficiente en ese depósito, y `PedidoMovilService` rechaza el pedido (422) si igual llega. `Parametros.permiteStockNegativo`. |
...
- **Modo repartidor (delivery de mercadería, modo API)**: sub-modo de la sesión API. El ERP (módulo `Reparto`) expone `/api/<tenant>/reparto/*`; el token Sanctum lleva ability `reparto` (no `preventa`) y solo lo emite si el `StaffUser` tiene rol `repartidor`. Se activa por **QR** (`data['modo'] == 'reparto'` en `QrPairingScreen` → `AuthProvider.adoptarSesionRepartoQr`) o por **login** (checkbox "Ingresar como repartidor" → `AuthProvider.loginRepartoConApi`). Persiste `api_role = 'repartidor'` (`ApiConfig.role()/isRepartidor()`); `AuthProvider.esRepartidor` gatea todo. **No hay base de catálogo** (`moviles_api.db`): `_AppRoot` saltea el gate de `DbInitState` cuando `isRepartidor()` y rutea a `RepartoHomeScreen` en vez de `HomeScreen`. Base propia **`reparto.db`** (`RepartoDatabaseHelper`, 100% de la app, no la pisa ninguna sync): `hoja_ruta`/`parada`/`parada_renglon` se reemplazan enteras en cada `fetchHojas`; `entrega_outbox` (`uuid` de idempotencia) es la cola de confirmaciones offline. `RepartoApi` (`lib/core/api/reparto_api.dart`): `pairWithCode`, `login`, `fetchHojas`, `iniciarHoja`, `paradaEnCamino`, `confirmarEntrega`, `reprogramar`. `RepartoProvider` orquesta: `sincronizar()` baja hojas + flushea el outbox; `confirmarEntrega()` encola + POST idempotente + estado local optimista + genera devolución de remito en el ERP por lo no entregado (parcial/rechazada). Pantallas en `lib/presentation/screens/reparto/`: `RepartoHomeScreen` (lista de hojas, pull-to-refresh, chip de pendientes), `HojaRutaScreen` (paradas en orden + "Iniciar reparto"), `ParadaScreen` (detalle + "En camino"/"Confirmar"/"Reprogramar"), `ConfirmarEntregaScreen` (segmented completa/parcial/rechazada; nombre+DNI; pad de firma `signature` **o** toggle "dejado sin firma" → cámara `image_picker` con ≥1 foto obligatoria; geo con `geolocator`). Plugins nuevos: `image_picker`, `geolocator` (+ permisos `ACCESS_FINE/COARSE_LOCATION` en `AndroidManifest.xml`; `CAMERA` ya estaba). Prefs: suma `api_role`. **Mapa** (`HojaRutaMapaScreen`): `flutter_map` + tiles OSM (gratis, sin API key) dibuja el recorrido (`HojaRuta.recorrido` ← `hoja.recorrido` del payload, polyline OSRM), paradas numeradas y "mi ubicación"; tap en parada → panel "Navegar" (abre Google Maps/Waze vía `url_launcher`, `lib/core/nav_launcher.dart`: `google.navigation:` → `geo:` → web) + "Ver parada". Botón de mapa en `HojaRutaScreen`, "Navegar" en `ParadaScreen`. Plugins: `flutter_map`, `latlong2`, `url_launcher` (+ `<queries>` `geo`/`https` en el manifest). **Pull-to-refresh** (`RefreshIndicator` → `RepartoProvider.sincronizar()`) en `RepartoHomeScreen`, `HojaRutaScreen`, `ParadaScreen`. **`ApiServerCard` en modo repartidor** (`ApiConfig.isRepartidor()`): oculta "Sincronizar catálogo" (403 con token `reparto`), "Reintentar envíos pendientes" y avisos de catálogo; muestra "Sincronizar hojas de ruta ahora".

Prefs del modo API: `api_base_url`, `api_tenant`, `api_token`, `api_last_sync`, `api_role`.
...
- `RepartoProvider` — modo repartidor: `sincronizar()`, `iniciarHoja()`, `marcarEnCamino()`, `confirmarEntrega()`, `reprogramar()`, `reintentarPendientes()`; getters `hojas` / `pendientes` / `syncing` (ver sección "Modo repartidor")
